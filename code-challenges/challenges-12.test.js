...
const replaceZeros = (string) => {
  ...
  let newZero = string.replace(regex, 'zero');
...
const validatePin = (pin) => {
  ...
  arr.forEach((item) => {
    ...
    if (parseInt(item) / parseInt(item) === 1) {
      newArr.push(item);
    }
  });

  return (newArr.length === 4) ? true : false;
};
...
const validateWord = (word) => {
  ...
  if (typeof (word) === 'string') {
    ...
    if (newArr.length >= 5 && newArr.length <= 10) {
      torf = true;
    } else {
      torf = false;
    }
